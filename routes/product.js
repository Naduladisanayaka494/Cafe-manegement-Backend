...
router.get('/getById/:id',(req,res,next)=>{
    ...
    connection.query(query,[id],(err,results)=>{
        if(!err){
            ...
            return res.status(500).json(results[0]);
...
router.patch('/update',(req,res,next)=>{
    ...
    connection.query(query,[product.name,product.categoryId,product.description,product.price,product.id],(err,results)=>{
        if(!err){
            ...
            return res.status(404).json({message:"product Updated Succesfully"});
...
router.patch('/updateStatus',(req,res,next)=>{
    ...
    connection.query(query,[user.status,user.id],(err,results)=>{
        if(!err){
            ...
            return res.status(404).json({message:"product status updated succesfully"})
