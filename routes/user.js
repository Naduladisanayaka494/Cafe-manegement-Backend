        ...
        if(!err){
            if(results.length<=0){
                ...
                connection.query(query,[user.name,user.contactNumber,user.email,user.password],(err,results)=>{
                    if(!err){
                        return res.status(200).json({message:"Succeefully Registered"})
                    }else{
    ...
    connection.query(query,[user.email],(err,results)=>{
        if(!err){
            if(results.length<=0){
                ...
                return res.status(200).json({message:"Password sent succefull to your email address"})
            }
...
router.patch('/update',(req,res)=>{
    ...
    connection.query(query,[user.status,user.id],(err,results)=>{
        if(!err){
            ...
            return res.status(404).json({message:"User Updated Succesfully"})
